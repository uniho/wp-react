...
const Page = props => {
    ...
    try {
      try {
        const r = await post2rest(
          '/?rest_route=/unsta/v1/api/modify-mail1/-', {
            mail: state.user,
          },
        )
  ...
  <div className=${cx(cssPage, 'flex j-center')}>
    <div className='login-form shadow fade-in animation-delay0'>
      ...
      <button className="btn--flat mt-4 w-full" onClick=${doSetMail}>
        メールアドレスを変更する
      </button>
      ...
      <button className="btn--raised2 mt-8 w-full" onClick=${doModify}>
        変更する
      </button>
    </div>
  </div>
